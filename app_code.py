...
function Waypoint() {
  ...
  const exportTripPDF = (trip, countryData) => {
    ...
    const margin = 18;
    const navy = [36, 87, 230];
    const gold = [255, 138, 61];
    const inkSoft = [88, 101, 121];
    const ink = [20, 33, 61];
    const hairline = [227, 232, 239];
    const parchment = [246, 248, 252];
    const accentRgb = (() => {
    ...
    doc.setFillColor(parchment[0], parchment[1], parchment[2]);
    doc.rect(0, 0, pageWidth, pageHeight, "F");
    compassMark(pageWidth / 2, pageHeight / 2 - 26, 9, navy, gold);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(22);
    doc.setTextColor(ink[0], ink[1], ink[2]);
